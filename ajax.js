// This Ajax example will return the current time (taken from the example PHP script get_time.php) and output it in the 
// second field of index.html.
// 
// It first tests for browser support, then stores a function in the method that processes the response from the server.
// 
// Then it will make a request to the server to get the data from get_time.php which currently just echos the server's 
// current time. If you were to edit get_time.php and just write "hihihi" index.html's second input field would display 
// "hihihi"
...
function ajaxFunction(){ 
	...
	ajaxRequest.onreadystatechange = function() {
		...
		// When readyState is 4, that means a response is complete. So we're going to check to see if readyState is 4 and 
		// when it is, we will do things:
